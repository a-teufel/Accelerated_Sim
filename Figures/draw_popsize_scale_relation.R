...
last<-ggplot(data=temp, aes(x=x, y=y)) +geom_point()+xlab(expression(paste("Original Sampling ",N[e])))+ylab(expression(paste("Accelerated Sampling Rescaled ", N[e])))+xlim(0,100)+ylim(0,100)+geom_smooth(method="lm",se=FALSE,colour="red",fullrange=TRUE)
